...
页面以「**院校专业组**」为主视角（这正是北京志愿填报与投档的单位）：输入分数或位次，
按 **冲 / 稳 / 保** 三档给出可报范围，展开后是专业组的招生专业、计划数、学制与学费。

数据来源分两条线：

| 数据 | 来源 | 形式 |
|------|------|------|
| 招生计划、选科要求、学制、学费 | 北京教育考试院综合查询系统 | HTML 表格 |
| **录取投档线（专业组级）** | 考试院「高考高招 > 通知公告」 | **PDF 附件** |
| **一分一段表（位次基准）** | 同上 | **PDF 附件** |
...
- [三、数据库设计](#三数据库设计)
- [四、采集层](#四采集层)
- [四之二、投档线 / 一分一段（PDF 附件）](#四之二投档线--一分一段pdf-附件)
...
│   ├── bjeea_pdf.py               # 投档线 / 分数分布 PDF 采集与解析 ★主数据来源
...
│   ├── ingest_pdf.py              # PDF 附件采集入口（投档线 / 一分一段，支持 --local 离线）
│   └── serve.py                   # 本地预览静态服务器
│
├── data/
...
**为什么不能用 HTML 解析。** 考试院把最关键的两类数据做成了 PDF 附件，公告正文只有一行链接：

- 「2026年北京市高招本科普通批录取投档线」→ 52 页 / 1569 行
- 「北京市2026年高考考生分数分布」（一分一段表）→ 313 段

所以加了独立采集器 `crawler/bjeea_pdf.py`，入口是 `scripts/ingest_pdf.py`：

```bash
python scripts/ingest_pdf.py --years 2026,2025        # 联网：自动发现并解析
python scripts/ingest_pdf.py --local 投档线.pdf --kind score_line --year 2026   # 离线：手工喂文件
python scripts/ingest_pdf.py --local 投档线.pdf --kind score_line --year 2026 --dry-run  # 只看解析结果
```
...
| # | 现象 | 处理 |
|---|------|------|
| 1 | 相当一部分行**只公布总分**，没有单科成绩（北京大学、中国人民大学多条记录都是） | 单科设成可选；强制要求 4 个单科会把这些行整行丢掉 |
| 2 | 总分有时**比单科合计高 10 分** | 那是政策性加分。校验规则是 `0 ≤ 总分 − 单科合计 ≤ 20`，不是相等 |
| 3 | 专科批那一列不是选考要求，而是**专业名称**（`M0 会展策划与管理`） | 用选考科目白名单判定列语义，非选科要求的值进 `note`，不进 `subject_req` |
| 4 | 行首被水印字符污染：`京671 64 943`、`北667 75 1251` | 允许行首少量非数字字符，并强制「分数逐段递减」兜底 |
| 5 | 列表分页是 `list_347_2.html`，且分页链接用**单引号** | 不硬编模板，从第 1 页发现分页链接；正则同时接受单双引号 |
| 6 | 附件文件名是未编码中文，urllib 直接抛 `UnicodeEncodeError` | 统一走 `ascii_url()` 做百分号编码 |
| 7 | 考试院同时发布普通类 / 艺术类（综合分）/ 专科 / 体育多张分布表 | 只接受普通类全口径那张，其余记入 `skipped`，绝不混进同一张 `score_rank` |

### 数据落点：为什么单独建 `group_admissions`

投档线是**专业组级**的。硬塞进 `admissions` 的专业级自然键，会让同一份分数在组内每个专业上重复一次 ——
页面上看起来就像「每个专业都考了同一个分」。所以单独建表，前端在「专业组」这一层展示它，这也正是官方口径。

位次不采信外部文本，统一由 `score_rank` 换算。同一张表里可能同时存在
「精确到 1 分」的分段（来自 PDF）与「合并区间」分段（如 `698-750`，来自种子 CSV），
换算时按**最窄匹配段**取值，精确分段优先。
...
```json
{
  ...
  "groups": {
    "columns": ["school_code","school_name","batch","group_code","subject_req",
                "min_score","rank_min","prev_min_score","plan_count","major_count", ...],
    "rows": [["1021","北京大学","本科普通批","01","不限",688,189,686,50,2, ...]]
  }
}
```
...
- `admissions-{年}.json`：**按年分片**，前端只加载选中那一年（`latest_year` 默认选中最近一年）
  - `rows` = 专业级（招生计划目录）；`groups` = **专业组级（官方投档线）**，前端的主视角
- `data.js`：全量内联包。双击打开 `index.html` 时浏览器会拦截 `fetch`，`app.js` 会自动回落到它

### 页面结构

信息架构围绕考生真正的问题组织 ——「我这个分数能报哪儿」，而不是「给我一张大表」：

1. **查询区**：总分 / 位次（二选一，填一个自动清空另一个）+ 选科要求 + 录取批次 + 院校或专业关键词
2. **分档结果区**：
   - 填了分数/位次 → 按 **冲**（投档线高 15 分以内）/ **稳**（已达线）/ **保**（低 15 分以上）分区
   - 只填位次时，用当年一分一段表换算**等效分**再分档
   - 「只看可报范围」默认开启，隐藏差距过大的组
3. **卡片 = 院校专业组**（北京志愿填报与投档的真实单位），展开后是专业明细表（专业 / 计划 / 最低分 / 学制·学费）
4. **可分享状态**：年份、分数、位次、选科、批次、关键词、排序全部同步到 URL

### 样式

浅灰底 + 白色卡片，用「1px 边框 + 背景色差」分层而不堆阴影；正文 14px / 行高 1.6，
指标数字 17px 等宽对齐（`tabular-nums`）；冲/稳/保各有固定的语义色，
卡片左侧色条与分区标题共用同一套颜色；窄屏切换为单列卡片流，明细表横向滚动。
...
> **受限环境提示**：如果 `import bs4` / `import lxml` 失败（例如本机自带依赖目录是按另一个
> Python 版本编译的），用 `pip install --target .deps312 beautifulsoup4 lxml pdfplumber`
> 单装一份，然后 `PYTHONPATH=.deps312 python scripts/run_all.py --offline`。
...
| 来源 | 用途 | 现状 |
|------|------|------|
| `query.bjeea.cn/queryService/rest/plan/115` | 招生计划（学校、专业组、选科要求、计划数、学制、学费） | ✅ **620/620 所学校全部采集完成**，2026 年共 5,831 条专业计划 |
| 考试院通知公告 · **投档线 PDF** | 专业组级投档最低分 + 单科成绩 | ✅ 已接入，**2026 年 2,186 条 + 2025 年 1,986 条** |
| 考试院通知公告 · **分数分布 PDF** | 一分一段表（位次基准） | ✅ 已接入，2026 年 313 段 + 2025 年 319 段 |
| 各高校本科招生网 | 专业级录取分数 | ⏳ 需在 `data/seed/school_sources.csv` 注册学校源（专业级分数的唯一公开来源） |
| 阳光高考平台 / 公众号 | 难度舆情、录取数据补充 | ⏳ 走 DeepSeek Flash 抽取 |

### 当前库中的真实数据

- **620 所学校**（考试院 2026 招生计划列表，13 页全量）
- **5,831 条专业计划**（2026 年，620 所学校；含专业组代码、选科要求、学制、学费）
- **4,174 条专业组投档线**（2026 + 2025，全部来自考试院官方 PDF）
- **632 个一分一段分段**（2026 + 2025，其中 3,685 条投档线已换算出位次）

### ⚠️ 关于 `data/seed/demo/` 里的示例数据

示例数据（19 条）**已经清除**。原因是它现在会和真实投档线混在同一个页面上 ——
哪怕标了 `demo`，考生看到「北京大学 688 分」也不会去分辨它是不是演示值。真实数据到位后，
假数据就没有存在理由了。

文件仍保留在 `data/seed/demo/`，需要离线演示前端功能时可以再载入：
...
**载入后 `index.json` 会列出 `demo_years`，页面会显示醒目的橙色警示；这些数字不可作为志愿填报依据。**

### 已知限制与后续工作

1. **专业级录取分数仍然缺失**。考试院只公布到「专业组」，专业级最低/平均/最高分只能逐校从高校招生网采集（`school_sources.csv`，目前为空）。因此专业明细里的「最低分」列当前是 `—`，这是数据现状，不是页面 bug。
2. **2025 年没有专业目录**（`admissions` 里只有 2026）。所以切到 2025 年能看到该年的投档线，但展开专业组时没有专业明细。要补这一块，可接考试院「录取统计查询（2022–2025）」或《2025年高招相关统计资料》。
3. **2024 及更早年份尚未接入**。考试院公告列表只保留 6 页（约 2025 年初至今），更早的投档线需要从历史存档页回溯；`gk-stu.bjeea.cn` 的录取统计查询覆盖 2022–2025，是补齐历史的下一步。
4. **专科投档线是专业级，不是专业组级**，且原文含「北京市教育考试院」水印字符会零星插入专业名（如「管试理」）。目前已把专业名与选科要求分开存放、避免污染筛选器，但个别专业名可能仍带一个多余字符。
5. **批次控制线（本科线 / 特殊类型线 / 专科线）未采集**。公告标题为「北京市2026年普通高等学校招生录取最低控制分数线」，建议下一步接入，用于判断「是否过线」。
6. **`score/250717`、`admission/791612` 是个人查询**（要准考证号），不可用于批量采集 —— 已在代码中排除。
7. **未接入的高校源**：`school_sources.csv` 默认为空。各校发布节奏不统一，建议先少量接入并观察，而不是一次性铺开。

### 关于「用公众号文章补数据」的实测结论

公众号（尤其各校招生办、志愿填报机构）确实会发布「XX大学 2025 年在京各专业录取分数线」，
但**实测不适合作为分数线的主要数据源**：

| 环节 | 结论 |
|------|------|
| 发现文章 | ✅ 可行。`crawler/wechat_search.py` 走搜狗微信搜索 → 解析 `/link?url=` 跳转页里的 JS 片段 → 还原 `mp.weixin.qq.com/s?...` 真实地址。三步都已跑通 |
| 抓取正文 | ✅ 可行，`mp.weixin.qq.com` 对普通 UA 返回 200 |
| **提取分数** | ❌ **不可行**。抽样 6 篇「2025 年各专业录取分数线」文章：每篇含 35~66 张 `mmbiz.qpic.cn` 图片，但正文文本里只有 1~7 个三位数字 —— 分数表**是图片**，不是文字表格 |

也就是说：这条链路能拿到文章，但拿不到数字。要走通必须上 OCR，而中文密集表格的 OCR 错误率
落在「分数」这种一位数都不能错的字段上，风险不可接受。另外搜狗结果里混有江苏、黑龙江等
非京数据，按校名搜索的召回精度也需要人工复核。

**建议路径**（同为「考试院之外」的数据，但是文字且可核验）：

1. **高校本科招生网**（`data/seed/school_sources.csv`）——公众号那批图表的原始出处，多数是 HTML 表格；
2. **考试院「录取统计查询（2022–2025）」**（`gk-stu.bjeea.cn`）——官方、免登录，含专业组与专业方向层级。
...
# 投档线 / 一分一段（PDF 附件）
python scripts/ingest_pdf.py --years 2026,2025
